...
export default class Poster {
  ...
  async post(entryRow) {
    ...
    if (entry.description) {
      ...
      const elms = [...new Set(dom.window.document.querySelectorAll('img'))]; // Remove dup
  ...
  static async renderPost(sbotConn, entry, description, link, channels, mentions, postTemplate) {
    ...
    if (
      Poster.calculateMsgLength(
        Poster.applyPostTemplate(
          postTemplate, entry.title, description, link, channels,
        ),
        mentions,
      ) > 8192 - 200
    ) {
      ...
      await sbotConn.publish(
        {
          ...
          text: Poster.applyPostTemplate(
            postTemplate, title, desc, link, channels,
          ),
  ...
  static applyPostTemplate(postTemplate, title, description, link, channels) {
    const matchMap = {
      title,
      description,
      link,
    ...
    return postTemplate.replace(
      /{(title|description|link|channels)}/gm,
